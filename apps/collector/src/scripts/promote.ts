...
async function fetchInstruments(): Promise<InstrumentKeyRecord[]> {
	const res = await fetch(`${API_BASE_URL}/instruments/keys`);
	if (!res.ok) throw new Error(`GET /instruments/keys failed: ${res.status}`);
	return res.json() as Promise<InstrumentKeyRecord[]>;
}

async function createInstrument(payload: InstrumentPayload): Promise<number | null> {
	if (DRY_RUN) {
		console.log(
			`  [DRY RUN] Would POST /instruments { symbol: ${payload.symbol}, exchange: ${payload.exchange} }`,
		);
...
async function storePriceHistory(
	...
	if (DRY_RUN) {
		console.log(
			`  [DRY RUN] Would POST ${allPoints.length} price history records for instrument ${instrumentId}`,
		);
		return;
	}
...
async function storeDividends(
	...
	if (DRY_RUN) {
		console.log(
			`  [DRY RUN] Would POST ${allDividends.length} dividend records for instrument ${instrumentId}`,
		);
		return;
	}
...
async function markValidated(id: number, symbol: string): Promise<void> {
	...
	const res = await fetch(`${API_BASE_URL}/potential-instruments/${id}/validate`, {
		method: "PATCH",
	});
	if (res.status !== 204) {
...
async function runWorker(
	...
	try {
		...
		for (const potential of chunk) {
			...
			if (isShuttingDown) {
				console.log(
					`[Worker ${workerId}] Shutdown signal received — stopping without marking remaining items.`,
				);
				break;
			}
			...
			console.log(
				`[Worker ${workerId}] [${itemIndex}/${chunk.length}] ${potential.symbol} (${potential.exchange})`,
			);
			...
			try {
				...
				if (scrapeSuccess) {
					...
					console.log(
						`[Worker ${workerId}]   Scrape insufficient (${nonNull}/16 metrics) — ${itemElapsed}s`,
					);
				}
			} catch (err) {
				...
				const itemElapsed = ((Date.now() - itemStart) / 1000).toFixed(1);
				console.log(
					`[Worker ${workerId}]   Scrape failed: ${err instanceof Error ? err.message : err} — ${itemElapsed}s`,
				);
			}
			...
			if (scrapeSuccess) {
				...
				if (existingKeys.has(key)) {
					console.log(
						`[Worker ${workerId}]   Duplicate — already in instruments. Updating price history.`,
					);
					...
					if (existingId !== undefined && scrapeResult) {
						await storePriceHistory(
							...
							scrapeResult.daily,
							scrapeResult.weekly,
							scrapeResult.monthly,
						);
						await storeDividends(
							existingId,
							scrapeResult.daily,
							scrapeResult.weekly,
							scrapeResult.monthly,
					...
					console.log(
						`[Worker ${workerId}]   Progress: ${progressLine(counters.promoted + counters.skippedDupe + counters.failed, totalItems, runStart)}`,
					);
				} else {
...
console.log(
	`=== Promote Instruments (DRY_RUN=${DRY_RUN}, WORKERS=${WORKERS}, HEADLESS=${HEADLESS}, BATCH_SIZE=${BATCH_SIZE || "unlimited"}) ===\n`,
);

const [potentials, existing] = await Promise.all([fetchPotentialInstruments(), fetchInstruments()]);

const unvalidated = potentials.filter((p) => !p.validated);

console.log(
	`Fetched ${potentials.length} potential instruments (${unvalidated.length} unvalidated), ${existing.length} existing instruments.`,
);
...
if (BATCH_SIZE > 0) {
	console.log(
		`Batch limit applied: processing ${toProcess.length} of ${unvalidated.length} unvalidated symbols.`,
	);
}

const existingKeys = new Set<string>(existing.map((i) => dupeKey(i.symbol, i.exchange)));
const existingIdMap = new Map<string, number>(
	existing.map((i) => [dupeKey(i.symbol, i.exchange), i.id]),
);
...
const itemsPerMin =
	totalProcessed > 0 && totalElapsedSec > 0
